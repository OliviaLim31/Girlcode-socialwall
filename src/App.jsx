...
function App() {
  ...
  return <><div className="ambient ambient-a" /><div className="ambient ambient-b" /><main className="shell">
    <header className="browser site-nav"><a className="site-brand" href="#top" onClick={(event) => { event.preventDefault(); scrollTo({ top: 0, behavior: "smooth" }); }}><span className="site-brand-mark">✦</span><span>girlcode social wall</span></a><label className="site-search"><span aria-hidden="true">⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} type="search" placeholder="find someone..." autoComplete="off" aria-label="Find someone" /></label><button className="nav-add" onClick={() => formDialog.current.showModal()}>+ add me</button></header>
    <section className="hero"><div className="eyebrow">girls' vibe coding · kl · 12.09.26</div><h1>we met <em>irl.</em><br />stay <span className="highlight-wrap"><span className="highlight">connected</span></span> <em>online.</em></h1><p className="sub">a tiny people wall for the girls in the room ♡</p><TextLoop text="Create ✦ Learn ✦ Build ✦ Support" shape="wave" speed={90} direction="forward" separator="✦" curviness={62} fontSize={50} fontWeight={800} letterSpacing={2} uppercase color="#ffffff" ribbon ribbonColor="#d027ff" ribbonWidth={66} /><div className="hero-actions"><button className="primary" onClick={() => formDialog.current.showModal()}>+ add me</button><button className="ghost" onClick={copyLink}>copy page link</button></div></section>
    <section className="toolbar"><div><div className="tiny">who's here?</div><div className="count"><span>{shownPeople.length}</span> girls on the wall</div></div></section>
