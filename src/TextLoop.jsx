...
export default function TextLoop({
  ...
  const repeatedText = useMemo(
    () =>
      Array.from(
        { length: 7 },
        () => `${words.join(` ${separator} `)} ${separator}`,
      ).join("   "),
    [words, separator],
  );
  ...
  useLayoutEffect(() => {
    ...
    const tween = gsap.fromTo(
      textPath,
      { attr: { startOffset: start } },
      {
        attr: { startOffset: end },
  ...
  return (
    <div
      ref={root}
      className="text-loop text-loop--wave"
      style={{
        "--loop-color": color,
        "--ribbon-color": ribbonColor,
        "--ribbon-width": ribbonWidth,
        "--loop-size": `${fontSize}px`,
        "--loop-weight": fontWeight,
        "--loop-spacing": `${letterSpacing}px`,
      }}
      aria-label={text}
    >
      <svg
      ...
      >
        ...
        <text className="text-loop__text">
